...
def QueryControl : QueryResult → Prop
  | .single _ => True
  | .incremental initial subsequent =>
      initial.pending ≠ []
      ∧ (QueryResult.incremental initial subsequent).idsEventuallyComplete
...
causality.
-/
theorem deliveryComplete_iff_idUsageValid_control (result : QueryResult)
    : result.deliveryComplete = true ↔ result.idUsageValid ∧ QueryControl result := by
  cases result with
  | single response =>
      simp [QueryResult.deliveryComplete, QueryResult.idUsageValid, QueryControl]
  | incremental initial subsequent =>
      constructor
      · intro h
      ...
      · rintro ⟨safe, nonempty, live, initialNext, subsequentNext⟩
        ...
        simp only [QueryResult.deliveryComplete, Bool.and_eq_true, and_assoc]
