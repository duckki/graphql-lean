...
-/
def StreamDependenciesCompleted (work : Work) (events : List WorkEvent) : Prop :=
  ∀ node dependencies producer,
    NodeAt work node .stream dependencies producer
...
theorem StreamDependenciesCompleted.append {work events}
    (closed : StreamDependenciesCompleted work events) (tail : List WorkEvent)
...
theorem stream_dependencies_of_completed
    {work initial matching events failed node dependencies producer}
  ...
  · exact Or.inl empty
  · have existsHealthy : ∃ key ∈ dependencies, ¬NodeFailed work failed key := by
      apply Classical.byContradiction
      intro absent
      apply healthy
      apply NodeFailed.streamDependencies known empty
      intro key member
