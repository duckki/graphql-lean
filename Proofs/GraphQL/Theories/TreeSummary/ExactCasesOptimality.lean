...
The public concrete side is an independent derivation relation. This module relates that
relation to the executable cursor and then transports localized best-bound obligations.
-/
...
-- Proof-only relational interpretation used to connect the independent derivations to
-- the generic algebra-relation theorem. It is not part of the public outcome semantics.
@[reducible]
...
private def relation {semantics : OutcomeSemantics.{u}} {abstract : Algebra.{v}}
  ...
  {
    related := BestBound laws.le laws.approximates
...
private abbrev Matches (environment : BooleanEnvironment) (assignment : BooleanAssignment)
    : Prop :=
...
private theorem exists_matches (environment : BooleanEnvironment)
...
private theorem matches_assign_iff
...
@[simp]
private theorem pruningValues_assign (environment : BooleanEnvironment)
    (variableName : Name) (value : Bool)
...
-- The executable fold collects exactly the independently derived cursor outcomes.
...
-- The exact traversal is the least abstract bound of the independently defined
-- feasible cases under the supplied local transfer laws.
