...
theorem collectFlatFields_append_local {ObjectRef : Type} (schema : Schema)
    ...
    (source : ResolverValue ObjectRef) (left right : List Selection)
...
def knownBooleanVariableNames (variableValues : VariableValues) : List Name :=
  (variableValues.map Prod.fst).filter
    fun variableName =>
...
theorem inputValueBoolean?_eq_of_agree_of_complete
    ...
    (hcomplete : ∃ value, inputValueBoolean? first (.variable variableName) = some value)
...
theorem guardedFieldGroupFor_ofTypeRegionUnder_booleanVariablesWithin
    (schema : Schema) (region : List Name)
    (leftContributions rightContributions : List (List BooleanLiteral × List Selection))
    (variables : List Name)
    (hleftSeed
      : ∀ condition selectionSet,
  ...
  · cases hfind
          : findGuardedFieldGroup? right.responseName
              (guardedFieldGroups
                (SelectionConditions.ofTypeRegionUnder schema region
                  leftContributions)) with
    | none =>
...
theorem guardedFieldGroupFor_entry_origin
    ...
    {entry : ConditionedField}
    (hentry : entry ∈ (guardedFieldGroupFor (guardedFieldGroups entries) right).entries)
    : entry ∈ entries := by
  cases hfind
        : findGuardedFieldGroup? right.responseName (guardedFieldGroups entries) with
  | none =>
      unfold guardedFieldGroupFor at hentry
      simp [hfind] at hentry
  | some left =>
...
mutual
  theorem guardedFieldGroupsIncludeWithFuel_entries_semantic_sound
      ...
      (hrightComplete
        : boolVarsComplete
            ...
            targetValues)
      (hleftChildrenComplete
        : ∀ entry,
            entry ∈ leftEntries
            -> boolVarsComplete
                (SelectionConditions.selectionSetBooleanVariables
                  entry.field.selectionSet)
                targetValues)
      (hrightChildrenComplete
        : ∀ entry,
            entry ∈ rightEntries
            -> boolVarsComplete
                (SelectionConditions.selectionSetBooleanVariables
    ...
    | succ childFuel =>
        ...
        have hcases : ∀ right,
          ...
          · intro candidateFuel heq fieldType hleftNonempty hrightNonempty
            ...
            have hleftLocal : (left.responseName, guardedLeftFields) ∈
              ...
              | nil =>
                  exact False.elim
                    (hleftNonempty (by simpa [left, guardedLeftFields] using hfields))
            ...
            have hrightLocal : (right.responseName, guardedRightFields) ∈
              ...
              | nil =>
                  exact False.elim
                    (hrightNonempty (by simpa [guardedRightFields] using hfields))
              | cons field rest => simp [executableFieldsAsGroup]
            ...
            have hrightCompletionReady
                : completionFieldsSemanticsReady schema runtimeType
                    fieldType runtimeRightFields :=
              ⟨
                hruntimeRightReady right.responseName runtimeRightFields
                  hruntimeRightGroup,
            ...
            · intro childValues hchildComplete hchildAgrees childRuntimeType hchildRuntime
                ...
                calc
                  activeContributionSelectionSet targetValues leftContributions
                    ...
                    activeContributionSelectionSet_guardedFieldChildContributions
                      targetValues runtimeType leftRuntimeEntries
                      (by
                        intro entry hentry
                  ...
                  activeContributionSelectionSet targetValues rightContributions
                      = executableFieldsMergedSelectionSet
                          (guardedFieldExecutableFields targetValues runtimeType
                            rightRuntimeEntries) :=
                    activeContributionSelectionSet_guardedFieldChildContributions
                      targetValues runtimeType rightRuntimeEntries
                      (by
                        intro entry hentry
                        exact List.contains_iff_mem.mp (List.mem_filter.mp hentry).2)
              ...
              exact ⟨
                hchildObject,
                rfl,
                (by simpa using hleftChildReady),
                (by simpa using hleftChildMerge),
                (by simpa using hrightChildReady),
                (by simpa using hrightChildMerge),
                hchildResult
              ⟩
        ...
        have hruntimeInclude : executableGroupsIncludeBool schema runtimeType
          ...
          refine executableGroupsIncludeBool_transport schema runtimeType
            ...
            (executableGroupsResolverReady_of_semanticsReady hruntimeRightReady) ?_
            hguarded
          ...
          have hchildObject : schema.objectType childRuntimeType :=
            SchemaWellFormedness.schemaWellFormed_possibleTypesAreObjects hschema
              fieldType.namedType childRuntimeType hchildRuntime
          have hleftCompletionReady
              : completionFieldsSemanticsReady schema runtimeType fieldType
                  runtimeLeftFields :=
            ⟨
              hruntimeLeftReady leftName runtimeLeftFields hruntimeLeftGroup,
              hruntimeLeftWitness
            ⟩
          have hrightCompletionReady
              : completionFieldsSemanticsReady schema runtimeType fieldType
                  runtimeRightFields :=
            ⟨
              hruntimeRightReady rightName runtimeRightFields hruntimeRightGroup,
              hruntimeRightWitness
            ⟩
          have hchildIncludesBool : schema.typeIncludesObjectBool fieldType.namedType
              childRuntimeType = true := List.contains_iff_mem.mpr hchildRuntime
          have hleftChildReady := completionFieldsSemanticsReady_merged_semantics
          ...
          have hleftChildComplete
            ...
            have htargetVariable :=
              mergedSelectionSet_variables_within_of_group_mem schema targetValues
                runtimeType runtimeType leftTargetSelectionSet
                (leftName, runtimeLeftFields) hruntimeLeftGroup variableName
                hruntimeVariable
            exact hleftComplete variableName htargetVariable
          have hrightChildComplete : boolVarsComplete
              (SelectionConditions.selectionSetBooleanVariables
  ...
  theorem guardedFieldGroupsIncludeWithFuel_semantic_sound
    ...
    exact guardedFieldGroupsIncludeWithFuel_entries_semantic_sound schema hschema
      ...
      (by simpa [leftEntries, rightEntries] using hcheck)
      hagrees hleftTargetComplete hrightTargetComplete
      (by
        intro entry hentry variableName hvariable
  ...
  theorem guardedFieldGroupsIncludeWithFuel_semantic_complete
      ...
      : guardedFieldGroupsIncludeWithFuel schema responseFuel
        ...
        = true := by
    let leftEntries :=
      SelectionConditions.ofTypeRegion schema parentRegion leftExtractedSelectionSet
    let rightEntries :=
      SelectionConditions.ofTypeRegion schema parentRegion rightExtractedSelectionSet
    let leftGroups := guardedFieldGroups leftEntries
    let rightGroups := guardedFieldGroups rightEntries
    ...
    · intro knownValues hknownValues hbaseAgrees right hright hgroupKnown
      ...
      have hrightEquivalent : RuntimeGroupsPermutationEquivalent
        ...
        exact selectionConditionsForRegion_runtimeGroups_permutationEquivalent schema
          parentRegion hrightSelectionSet targetValues runtimeType runtimeType PUnit.unit
          hruntimeParent
      have hruntimeLeftReady :=
        executableGroupsSemanticsReady_collectFields schema targetValues
          runtimeType PUnit.unit leftTargetSelectionSet hparentCase.1
          hparentCase.2.2.1 hparentCase.2.2.2.1
      have hruntimeRightReady :=
        executableGroupsSemanticsReady_collectFields schema targetValues
      ...
      | zero =>
          ...
          exact guardedFieldGroupCases_complete_runtime schema 0
            ...
            (by
              simpa [leftGroups] using guardedFieldGroups_responseNames_nodup leftEntries)
      ...
      | succ childFuel =>
          ...
          | cons rightHead rightRest =>
              ...
              have hguardedLeftLocal
                  : (leftName, guardedLeftFields') ∈ guardedLeftLocalGroups := by
                apply guardedFieldGroupFor_runtimeGroup_mem targetValues runtimeType
                  leftGroups right
                  (by
                    simpa [leftGroups]
                      using guardedFieldGroups_responseNames_nodup leftEntries)
              ...
              have hruntimeSingletonInclude
                  : executableGroupsIncludeBool schema runtimeType
                      (fun outputType leftSelectionSet rightSelectionSet =>
                        (schema.getPossibleTypes outputType.namedType).all
                          fun candidateRuntimeType =>
                            selectionSetIncludesBoolWithFuel schema childFuel
                ...
                simpa [executableGroupsIncludeBool, executableGroupIncludedBool]
                  using hruntimeMatch
              have hruntimeLeftSingletonReady
                  : executableGroupsSemanticsReady schema runtimeType
              ...
              have hruntimeRightSingletonReady
                  : executableGroupsSemanticsReady schema runtimeType
                ...
                subst responseName
                subst fields
                exact hruntimeRightReady right.responseName runtimeRightFields
                  hruntimeRightGroup
              have hguardedSingletonInclude
                  : executableGroupsIncludeBool schema runtimeType
                      (fun outputType leftSelectionSet rightSelectionSet =>
                        guardedFieldChildIncludesBool schema childFuel knownValues
                          (schema.getPossibleTypes outputType.namedType) leftSelectionSet
                ...
                · intro fieldType candidateLeftName candidateRightName
                  ...
                  · intro variableName hvariable
                    ...
                    have htargetVariable :=
                      mergedSelectionSet_variables_within_of_group_mem
                        schema targetValues runtimeType runtimeType leftTargetSelectionSet
                        (leftName, runtimeLeftFields)
                  ...
                  · intro variableName hvariable
                    ...
                    have htargetVariable :=
                      mergedSelectionSet_variables_within_of_group_mem schema targetValues
                        runtimeType runtimeType rightTargetSelectionSet
                        (right.responseName, runtimeRightFields)
                  ...
                  · intro childValues hchildComplete hchildAgrees childRuntimeType
                    ...
                    rcases hcases childValues hchildComplete houterAgrees runtimeType
                      ...
                      ⟨childOuterLeftSelectionSet, childOuterRightSelectionSet,
                        ...
                        houterCase⟩
                    let childRuntimeLeftGroups :=
                      collectRuntimeFieldGroups schema childValues
                        runtimeType runtimeType childOuterLeftSelectionSet
                    let childRuntimeRightGroups :=
                      collectRuntimeFieldGroups schema childValues
                    ...
                    have hchildLeftEquivalent
                      ...
                      exact selectionConditionsForRegion_runtimeGroups_permutationEquivalent
                        schema parentRegion hchildOuterLeftSelectionSet childValues
                        runtimeType runtimeType PUnit.unit hruntimeParent
                    have hchildRightEquivalent
                        : RuntimeGroupsPermutationEquivalent
                            (guardedFieldRuntimeGroups childValues runtimeType
                              rightGroups)
                      ...
                      exact selectionConditionsForRegion_runtimeGroups_permutationEquivalent
                        schema parentRegion hchildOuterRightSelectionSet childValues
                        runtimeType runtimeType PUnit.unit hruntimeParent
              ...
              have hlocalIncluded :=
                executableGroupIncludedBool_mono_left schema runtimeType
                  (fun outputType leftSelectionSet rightSelectionSet =>
                    guardedFieldChildIncludesBool schema childFuel knownValues
                      (schema.getPossibleTypes outputType.namedType) leftSelectionSet
                      rightSelectionSet)
                  ...
                  (by
                    intro group hgroup
                    have heq : group = (leftName, guardedLeftFields') := by
                      simpa using hgroup
                    subst group
